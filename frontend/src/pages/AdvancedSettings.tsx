...
import type { FreqConfig, GatingConfig, ModelConfig, DetectionConfig, IngestConfig, SourcesConfig } from '../types';
...
export default function AdvancedSettings({ onBack }: { onBack: () => void }) {
  ...
  const saveSources = async () => {
    if (!sources) return;
    const clean: SourcesConfig = { facility: {}, hostname: {}, tag: {} };
    for (const sec of ['facility', 'hostname', 'tag'] as const) {
  ...
  return (
    <div className="page">
      ...
      <div className="page-head"><h2>高级设置</h2><span className="sub">阈值 · 模型 · 检测调参</span></div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div className="card">
          <div className="sec-label">Mock 规则（仅 mock 模式生效）</div>
        ...
        <div className="card">
          <div className="sec-label">接入与来源</div>
          {ingest && (
            <div className="field-row" style={{ marginTop: 10 }}>
          ...
          <p className="muted" style={{ marginTop: 6 }}>syslog 地址/端口改后需重启后端；巩固间隔与 API token 立即生效。</p>
          {sources && (
            <div style={{ marginTop: 12 }}>
              <div className="muted" style={{ fontWeight: 600, marginBottom: 6 }}>syslog 来源映射（优先级 tag &gt; hostname &gt; facility，子串匹配）</div>
              ...
              <button className="btn primary" onClick={saveSources} style={{ marginTop: 8 }}>保存来源映射</button>
            </div>
          )}
        </div>
