...
export default function Settings({ user }: { user: AuthUser }) {
  ...
  const canConfig = hasPerm(user, 'config');
  const canMaintain = hasPerm(user, 'maintenance');
  ...
  useEffect(() => { if (canConfig || canMaintain) load(); }, [canConfig, canMaintain]);
  ...
  if (canConfig && tab === 'advanced') return <AdvancedSettings onBack={goBasic} />;

  return (
    <div className="page">
      <div className="page-head">
        <h2>{t('settings')}</h2>
        {canConfig && <span className="sub">{t('knob')} · 模型模式 · 接入</span>}
        <div className="spacer" />
        {canConfig && <button className="btn" onClick={() => setTab('advanced')}>高级设置 →</button>}
      </div>
