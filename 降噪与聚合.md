...
| 降噪 | `prototype/signature.py` | `signature(source, type_, raw)` |
...
def signature(source, type_, raw):
    s = _IP.sub("<IP>", raw or "")
    s = _HASH.sub("<HASH>", s)
    s = _NUM.sub("<NUM>", s)
...
`prototype/tolerance.py`。数据结构是 `set[str]`，持久化到 `data/tolerance.json`。

```python
def is_tolerated(signal, entries: set[str]) -> bool:
    return signature(signal["source"], signal["type"], signal["raw"]) in entries
```

- 命中 → `process_signal` 直接 `_suppress(...)`，**不调用任何模型**，落库 `suppressed=1`。
- 写入：分析师标「误报」时，`learn()` 把该案所有告警的签名并入集合。
...
def match(signal, rules: set[str]) -> bool:
    return signature(signal["source"], signal["type"], signal["raw"]) in rules
```
...
| 入口 | 路径 | 调用 |
|---|---|---|
| `pipeline.process(signals, knob)` | 批量 | CLI `backend/ingest.py`（一次性读文件建图） |
| `pipeline.process_signal(signal)` | 流式 | `syslog_server.py`、`api/dashboard.py` 上传端点（24h 主路径） |
| `pipeline.restore_signal(signal)` | 放回 | `api/dashboard.py`（被抑制信号放回） |

差异：频率降级与「归案/合并」只在 `process_signal`（流式）；`process`（批量）是
一次建图 + 连通分量归案，无逐条增量。
